	...
	try {
        ...
        const sendStickerFromUrl = async(to, url) => {
            ...
            if(mime.split("/")[0] === "audio"){
		        ...
		        await haruka.sendMessage(from, `LINK GROUP TERDETEKSI!!, Maaf bot akan kick lu, yahahaha`, text , {quoted: mek})
		        haruka.groupRemove(from, [kic]).catch((e)=>{reply(`Adminin bot dulu njer😒`)})
		        }
			...
			if (tebakgambar.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
                ...
                if (budy.toLowerCase() == jawaban) {
                    sendButMessage(from, "Selamat 🎉 Jawaban lu benar!", `• ${ownername}`, [{"buttonId": `.tebakgambar`,"buttonText": {"displayText": "Tebak Gambar"},"type": "RESPONSE"}], {quoted : mek})
                    delete tebakgambar[sender.split('@')[0]]
                    fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
                } else {
                    reply("Jawaban Salah Awokawok!")
                }
            }
		...
		//console termux
		if(!(isCmd)){
			console.log(('|\x1b[1;33m PESAN \x1b[1;33m|'), time, chalk.yellow(budy), 'from', chalk.green(pushname), 'args :', chalk.green(args.length), 'in', chalk.green(groupName? groupName : 'Private chat'))
		}
		if(!(isCmd || mek.key.fromMe)){
			console.log(('|\x1b[1;32m PESAN CMD \x1b[1;37m|'), time, chalk.green(command), 'from', chalk.green(pushname), 'args :', chalk.green(args.length), 'in', chalk.green(groupName? groupName : 'Private chat'))
		}
...
switch (command) {
case 'menu': case 'help':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Ini kontak owner gw, jangan spam`, text, {quoted: hehe})
				}
			break
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
	...
	.then(async(result) => {
		for(let i of result.medias){
            ...
            }).catch((err) => reply(`🤲 Server eror yahaha:v`))
            
             break
case 'tiktok':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
sendButLocation(from, 'Silahkan pilih media yang ingin lu download', '© ' + ownername, thumbnail, [{buttonId: `.tiktokwm ${q}`, buttonText: {displayText: 'WM ⿻'}, type: 1},{buttonId: `.tiktoknowm ${q}`, buttonText:{displayText: 'NOWM ⿻'}, type: 1},{buttonId: `.tiktokmusic ${q}`, buttonText:{displayText: 'AUDIO'}, type: 1}], {quoted: mek})
						
             break
case 'tiktoknowm':   
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			if (!q) return reply('Link tiktoknya mana?')
			if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply('Link ga valid')
			reply(lang.wait())
			let nowem = q
			...
			.then(result => {
				...
				}).catch((err) => reply(`Link gak valid`))
...
case 'pinterest': 
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			if(!q) return reply('Masukkan contoh')
...
• Nama : ${aramas.videos[0].title}
• ID : ${aramas.videos[0].videoId}
• Upload : ${aramas.videos[0].ago}
• Ukuran : ${data.medias[7].formattedSize}
• Penonton: ${aramas.videos[0].views} 
• Durasi : ${aramas.videos[0].timestamp}
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('Udah aktif sebelumnya')
						antilink.push(from)
...
case 'memegenerator': case 'memegen':{
									...
									try {
									   ...
									   var mediiia = await haruka.downloadMediaMessage(enmedia)
										...
										haruka.sendMessage(from, resu, image, {caption:'.stikerin mek', thumbnail: Buffer.alloc(0), quoted: mek})
						...
						if (args.length < 1) return reply(`Kirim perintah *${prefix + command}* HanifGanteng`)
									if (q.includes('|')) return reply(`Kirim perintah *${prefix + command}* HanifGanteng`)
									try {
...
case 'leave':
			...
			setTimeout( () => {
			haruka.sendMessage(from, 'Beban Keluar👋', text)
			}, 0)
			break
case 'hidetag':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
					...
					haruka.sendMessage(from, `Sukses ubah nama group menjadi ${q}`, text, { quoted: mek })
			break          
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
					...
					haruka.sendMessage(from, `Sukses ubah deskripsi group menjadi ${q}`, text, { quoted: mek })
			break   
case 'kick':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
...
case 'bc': case 'broadcast':
			...
			var bcnya3 = await haruka.downloadMediaMessage(bcnya2)
					...
					break      
case 'nightcore':{
	                 if (!isQuotedAudio) return reply('Reply audio nya tod')
...
case 'toimg':{
		if (!isQuotedSticker) return reply('Reply stikernya nya asu!')
					...
					haruka.sendMessage(from, buffer, image, {quoted: mek, thumbnail:Buffer.alloc(0), caption: 'Done bwang'})
					fs.unlinkSync(ran)
					})
					}
					break   
case 'nuliskiri':{
									if (args.length < 1) return reply(`Kirim perintah *${prefix}nuliskiri* teksnya`)
									reply(lang.wait())
									const tulisan = q
									...
									break
						case 'nuliskanan':{
									if (args.length < 1) return reply(`Kirim perintah *${prefix}nuliskanan* teksnya`)
									reply(lang.wait())
									const tulisan = q
									const splitText = tulisan.replace(/(\S+\s*){1,9}/g, '$&\n')
									const fixHeight = splitText.split('\n').slice(0, 31).join('\n')
									...
									break
						case 'foliokiri':{
									if (args.length < 1) return reply(`Kirim perintah *${prefix}foliokiri* teksnya`)
									reply(lang.wait())
									const tulisan = q
									...
									break
						case 'foliokanan':{
									if (args.length < 1) return reply(`Kirim perintah *${prefix}foliokanan* teksnya`)
									reply(lang.wait())
									const tulisan = q
									const splitText = tulisan.replace(/(\S+\s*){1,13}/g, '$&\n')
									const fixHeight = splitText.split('\n').slice(0, 38).join('\n')
	...
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
                if(!q)return reply(`Contohnya : ${prefix + command} link Facebook`)
                if (!q.includes('facebook.com') && !q.includes('fb.watch')) return reply('Itu dudok link Facebook tolol')
...
try{
                zee.Facebook(`${q}`).then(async data => {
             	...
             	reply('Fitur Masih emror')
} 
   }          
             break
   case 'soundcloud':
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
                if(!q)return reply(`Contohnya : ${prefix + command} link SoundCloud`)
				...
				zee.SoundCloud(`${q}`).then(async (data) => {
                    ...
                    txt += `*• Nama :* ${data.title}\n`
                    txt += `*• Durasi :* ${data.duration}\n`
					txt += `*• Quality :* ${data.medias[1].quality}\n`
					txt += `*• Ext :* ${data.medias[0].extension}\n`
                    txt += `*• Size :* ${data.medias[0].formattedSize}\n`
                    txt += `*• Url  :* ${data.url}\n\n`
                    txt += `*Entenono masih di proses...*`
	...
	case 'telesticker': case 'tstiker': {
		if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			...
			if (!q.includes('t.me')) return reply('Bukan link telegram stiker kon')
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
if (!q) return reply('emojinya apa?')
					qes = args.join(' ')
reply(lang.wait())
...
case 'ytmp3': {
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Tekan button ini untuk daftar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar ⿻`,},type: 1,}], {quoted: fgif});
			if (args.length === 0) return reply(`Kirim perintah *${prefix}ytmp3* _Link YouTube_`)
			if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('Link gak valid!')
